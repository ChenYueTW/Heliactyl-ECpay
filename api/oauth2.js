	...
	app.get(settings.api.client.oauth2.callbackpath, async (req, res) => {
		...
		res.send(`
    		...
    		<body style="background-color: #111319; font-family: 'IBM Plex Sans', sans-serif;">
    			<center>
      				<br><br><br>
      				<h1 style="color: white">Logging...</h1>
      				<p style="color: #BBBBBB">請稍候，很快就會被重定向</p>
    			</center>
						...
						if (await accountjson.status == 201) {
							...
							if (user.length == 1) {
								...
								if (userids.filter(id => id == userid).length == 0) {
									...
									return res.send("偵測到一個帳戶包含您的 Discord 電子郵件，但該使用者 ID 已被另一個 Discord 帳戶佔用");
								}
							} else {
								return res.send("嘗試建立您的帳戶時發生錯誤");
							};
						};
						log('新帳號創建', `\`\`\`UserName: ${userinfo.username}\nGlobalName: ${userinfo.global_name}\nID: ${userinfo.id}\nIP: ${realIp}\`\`\``)
					} else {
						return res.send("新用戶目前無法註冊")
					}
				} else {
					log('用戶登入', `\`\`\`UserName: ${userinfo.username}\nGlobalName: ${userinfo.global_name}\nID: ${userinfo.id}\nIP: ${realIp}\`\`\``);
				}
